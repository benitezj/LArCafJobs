...
bool  HistoryContainer::clearData()
{
  
  //cout<<"HistoryContainer::clearData()  m_data.size = "<<m_data.size()<<endl;
  for (std::vector<const DataContainer*>::iterator data = m_data.begin(); data != m_data.end(); data++) 
    if (*data) delete *data;
  m_data.clear();
