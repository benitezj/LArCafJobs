    ...
    m_historyCont(0), m_eventData(0), m_runData(0)
{
  ...
  m_cellTree = new TTree("cells", ""); m_cellTree->SetDirectory(m_file);
  m_eventTree = new TTree("events", ""); m_eventTree->SetDirectory(m_file);
  m_runTree = new TTree("runs", ""); m_runTree->SetDirectory(m_file);
...
{
  if(!cont){ cout << "ERROR : HistoryContainer is NULL : " << cont <<". cellTree not filled." << endl; return;}
  m_historyCont = cont;
  m_cellTree->Fill();
  m_historyCont = 0;
}
